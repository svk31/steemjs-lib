    ...
    */
    ...
    // return this.get_potential_signatures().then( (pubkeys)=> {
        ...
        return this.get_required_signatures(my_pubkeys).then( required_pubkeys => {
            ...
            if (!tr.private_signatures.length) {
                throw new Error("Unable to sign transaction, no matching keys found");
            }
...
function _broadcast(was_broadcast_callback){
    return new Promise((resolve, reject)=> {
        ...
        Api.network_broadcast_api().exec( "broadcast_transaction_with_callback", [ function(res) { return resolve(res); } ,tr_object]).then(function(){
